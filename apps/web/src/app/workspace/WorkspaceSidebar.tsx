            ...
            <div key={cls.id}>
              ...
              {isOpen && (
                <div style={{ paddingBottom: 4 }}>
                  {/* Join link */}
                  {cls.joinCode && (
                    <div style={{ padding: "5px 18px 5px 30px", display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontSize: 10, color: "#555" }}>JOIN</span>
                      <span style={{ fontSize: 10, fontFamily: "monospace", color: "#4DB832", letterSpacing: ".08em" }}>{cls.joinCode}</span>
                      <button
                        onClick={() => navigator.clipboard.writeText(`${window.location.origin}/join/${cls.joinCode}`)}
                        title="Copy join link"
                        style={{ background: "none", border: "none", color: "#555", fontSize: 11, cursor: "pointer", padding: "0 2px" }}
                      >⧉</button>
                    </div>
                  )}
                  {cls.syllabus ? (
